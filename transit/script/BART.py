...
def process_BART_data(file_name, time, node, station):
    ...
    BART.columns = ['Node', 'Boardings', 'Node Name', 'Station', 'Alightings', 'TOD', 'Key']
    BART = BART[['Node', 'Node Name', 'Station', 'TOD', 'Key', 'Boardings', 'Alightings']]
...
counties = {
    ...
    'Contra Costa': ['RICH', 'ORIN', 'LAFY', 'WCRK', 'CONC', 'NCON', 'PITT', 'ANTC', 'DELN', 'PHIL', 'PCTR', 'PLZA'],
    'Alameda': ['WOAK', '12TH', '19TH', 'MCAR', 'ASHB', 'DUBL', 'WDUB', 'CAST', 'WARM', 'UCTY', 'SHAY',
                'HAYW', 'BAYF', 'SANL', 'OAKL', 'COLS', 'FTVL', 'LAKE', 'ROCK', 'DBRK', 'NBRK', 'FRMT'],
...
BART_county = BART_county.groupby(['County', 'TOD'])[['Boardings', 'Alightings']].sum().reset_index()
...
def BART_SL_Concat(df, Screenline):
    ...
    BART_SL_CT['Key'] = BART_SL_CT['Screenline'] + BART_SL_CT['Direction'] + BART_SL_CT['TOD']
...
def process_SL_data(path, lines, node_df, df_station_name, relevant_stations,TOD):
    ...
    values_to_drop = ['Hillcrest eBART', 'Coliseium OAC', 'Somersville Road eBART']
    intra_am = intra_am[~intra_am['A_name'].isin(values_to_drop) & ~intra_am['B_name'].isin(values_to_drop)]
    intra_am = intra_am[intra_am['A_station'].isin(relevant_stations) & intra_am['B_station'].isin(relevant_stations)]
