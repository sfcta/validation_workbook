...
file_check = [WORKING_FOLDER, AM_dbf, PM_dbf, MD_dbf, EV_dbf, EA_dbf, Line_Name_File, Line_Rename_File, Transit_Templet ]
...
name_to_trn_asgn_new = pd.Series(rename.Trn_asgn_new.values, index=rename.NAME).to_dict()
new_name_to_line = pd.Series(rename.new_name.values, index=rename.Trn_asgn_new).to_dict()

# Map the Name in df1 to Trn_asgn_new using the mapping
MUNI_full['Name'] = MUNI_full['Name'].map(name_to_trn_asgn_new).fillna(MUNI_full['Name'])
...
def transform_line(line):
    if pd.isna(line):
        ...
        return int(line)
    elif any(char.isdigit() for char in line) and any(char.isalpha() for char in line):  # Case 2: Numbers and letters
        if 'SHORT' in line:
...
MUNI_full['Key_line_tod'] = MUNI_full['Line'].astype(str) + MUNI_full['TOD'] + MUNI_full['Direction']
MUNI_full = MUNI_full[['Line', 'Mode', 'Direction', 'TOD', 'Key_line_dir', 'Key_line_tod', 'Ridership','Name']]
MUNI_full = MUNI_full.sort_values(by=['Line', 'Direction','TOD']).reset_index(drop=True)
