...
def process_BART_county(combined_gdf, model_run_dir, output_dir, model_BART_county, model_BART):
    BART_county = process_BART_data(combined_gdf, model_run_dir, output_dir, model_BART )
    station_to_county = {station: county for county, stations in counties.items() for station in stations}
...
def process_bart_screenline_data(combined_gdf, A, B):
    ...
    bart_screenline = read_dbf_and_groupby_sum(combined_gdf, "BART", ["A", "B", "TOD"], "AB_VOL")

    # Filter rows for IB (16510 to 16511)
    IB = bart_screenline[(bart_screenline["A"] == A) & (bart_screenline["B"] == B)].copy()
    IB["Direction"] = "IB"

    # Filter rows for OB (16511 to 16510)
    OB = bart_screenline[(bart_screenline["A"] == B) & (bart_screenline["B"] == A)].copy()
    OB["Direction"] = "OB"
...
def BART_Screenline_Concat(combined_gdf, A, B, Screenline):
    ...
    bart_screenline["Key"] = (
        bart_screenline["Screenline"] + bart_screenline["Direction"] + bart_screenline["TOD"]
    )
    ...
    bart_screenline = bart_screenline[["Screenline", "Direction", "TOD", "Key", "Ridership"]]
...
def process_BART_Screenline(combined_gdf, model_run_dir, output_dir, model_BART_Screenline):
    transbay_node = [16510, 16511]  # 16510 in, 16511 out
    BART_sl_tb = BART_Screenline_Concat(combined_gdf, transbay_node[0], transbay_node[1], 'Transbay')
    countyline_node = [16519, 16518]  # 16519n-- in, 16518 --out
    BART_sl_ct = BART_Screenline_Concat(combined_gdf, countyline_node[0], countyline_node[1], "Countyline")
