...
for df in tables:
    ...
    if 'Average Weekday UPT' in df.iloc[:, 0].values:
        ...
        average_upt = total_annual_upt / 260  # Fallback if 'Average Weekday UPT' is not found
...
AC = {'Operator': ['AC Transbay', 'AC Eastbay'], 
...
MUNI = {'Operator': ['MUNI-Bus', 'MUNI-Rail', 'MUNI_Cable'],
...
SCVTA = {'Operator': ['SCVTA-Bus', 'SCVTA-LRT'],
...
def assign_ferry_name(name):
    transbay =  ['90_FAOF', '90_FBHB', '90FOAF', '90_HBFB', '90_OAFW', '90_OAKOP', '90_OPOAK', '90_WFAO']
    GG_transit = ['91_LARKN', '91_LARKS', '92_FBSAU', '92_SAUFB', '93_FBTIB', '93_FWSAU', '93_FWTIB', '93_SAUFW', '93_TIBFB', '93_TIBFW']
    if name in transbay:
        return 'SF Bay Ferry'
...
model_dic = {'Operator':['GGT-Ferry', 'GGT-Bus', 'SCVTA-Bus','SCVTA-LRT','SF Bay Ferry (WETA)', 'Muni-Bus', 'Muni-rail', 'Muni-Cable', 'AC Transbay', 'AC Eastbay'],
            'Modeled':[model_GG_ferry,GG_bus_model, VTA_bus_model, VTA_LR_model, model_SF_Bay, model_muni_bus, model_muni_rail, model_muni_cable, ac_trans, ac_east]}
df_model_dic = pd.DataFrame(model_dic)

df_modeled = pd.concat([all, df_model_dic])
df = pd.read_excel(Transit_Templet, usecols = "C:D", sheet_name='val_Tot', skiprows = list(range(4))+list(range(37,51)))
...
df_sf_ferry = pd.DataFrame({'Operator':['SF Bay Ferry (WETA)'],'Modeled': [model_SF_Bay]})
model_operator = pd.concat([model_operator, df_sf_ferry])

observal_operator['Operator'] = observal_operator['Operator'].map(name_mapping).fillna(observal_operator['Operator'])
model_operator['Operator'] = model_operator['Operator'].map(name_mapping3).fillna(model_operator['Operator'])
...
total_operator = format_dataframe(df_operator,['Observed', 'Modeled', 'Diff'], ['Pct Diff'])
dataframe_to_markdown(total_operator, file_name=os.path.join(OUTPUT_FOLDER, 'valTotal_Operator2.md'), highlight_rows=[23], center_align_columns=None, column_widths = 100)
total_operator[:-1].to_csv(os.path.join(total_output_dir,"valTotal_Operator2.csv"), index=False)

df_filtered['Operator'] = df_filtered['Operator'].map(name_mapping).fillna(df_filtered['Operator'])
df_modeled['Operator'] = df_modeled['Operator'].map(name_mapping2).fillna(df_modeled['Operator'])
...
df_service_type = result_df[['Service Type','Observed','Modeled']].groupby('Service Type').sum().reset_index()
st_total_row = pd.Series(df_service_type[['Observed', 'Modeled']].sum(), name='Total')
df_service_type = pd.concat([df_service_type, st_total_row.to_frame().T], ignore_index=True)
...
total_service = format_dataframe(df_service_type,['Observed', 'Modeled', 'Diff'], ['Pct Diff'])
dataframe_to_markdown(total_service, file_name=os.path.join(OUTPUT_FOLDER, 'valTotal_Service.md'), highlight_rows=[5], center_align_columns=None, column_widths = 100)
total_service[:-1].to_csv(os.path.join(total_output_dir,"valTotal_Service.csv"), index=False)

total_val = format_dataframe(result_df,['Observed', 'Modeled', 'Diff'], ['Pct Diff'])
dataframe_to_markdown(total_val, file_name=os.path.join(OUTPUT_FOLDER, 'valTotal_Operator.md'), highlight_rows=[32], center_align_columns=['Operator'], column_widths = 100)